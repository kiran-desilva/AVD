...
perf.Sf = 0.1512 * v_stall_landing^2 * sind(theta_apprch); 
perf.Sfr = tfr * V_td; 
perf.Ktl = design_t_w (moderate power - mul; %t/w need to change? note mjst be neative
perf.Ka_l = (1.225 / (2 * wandb.Wland / wing.Sref)) * (mul * aero_analysis.wing.landing_CLmax - aero_analysis.drag.Cd0_landing - (aero_analysis.wing.landing_CLmax^2) / (pi * wing.AR *aero_analysis.drag.e_landing));
...
perf.frac_cruise2 = exp(-(parameters.alternate_range_km * 1000 * cruise2_c / 3600) / (parameters.cruise_mach * 295.07 * aero_analysis.wing.cruise_LoverD));  %cruise 2
perf.frac_loiter = exp(-(parameters.loiter_duration * 60 * loiter_c / 3600) / (aero_analysis.wing.loiter_LoverD)); % loiter using endurance eqn
...
perf.cruise1_Range = (parameters.cruise_mach * 295.07 / cruise1_c) * aero_analysis.wing.cruise_LoverD * log(perf.frac_cruise1);
