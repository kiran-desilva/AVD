...
Vinit = 0;
takeoff.v_stall = sqrt(2 * wandb.W0 / (1.225 * Sref * high_lift_design.Clmax_takeoff)); %need clmax takeoff
V_TO = 1.1 * takeoff.v_stall; %FAR-25  
mu = 0.03; % raymer dry asphalt runway 
rolltime = 3; %raymer

perf.Ka = (1.225 / (2 * wandb.W0 / wing_design.S_ref)) * (mu * high_lift_design.Clmax_takeoff - aer0_analysis.drag.Cd0_takeoff - (high_lift_design.Clmax_takeoff^2) / (pi * wing_design.AR * aero_analyis.drag.e_takeoff));
perf.Kt = (Thrust_max_takeoff / W0) - mu;
