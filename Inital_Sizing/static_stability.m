clear
clc

load('sizing.mat')
load('parameters.mat')
load('tailplane.mat')
load('wing.mat')
load('locations.mat')
load('fuse.mat')
load('aero_analysis')
load('design')
load('wing')

%% initial variables





Sw = wing.Sref; %cessna mustag CHANGE
xacw = locations.x_ac_w;
Cmac = wing.Cmac;
wing_sweep_25 = wing.sweep_25;
wing_twist = wing.twist;
wing_AR = wing.Ar;
wing_b = wing.b;
wing_xac_bar = xacw/Cmac;
wing_Croot = wing.Croot;
alpha_0_w = degtorad(-3);

eta_h = 1.0 ; % t-tail efficency -> errikos slides
Sh = tailplane.horizontal.s; % this prob should be have set in postions
zh = tailplane.horizontal.z_above_fuselage_bar .* Cmac;

lh = tailplane.horizontal.l;

tail_h_Cl_alpha = aero_analysis.tail.Cl_alpha(1); 
tail_h_xac_bar = locations.x_ac_h/Cmac;
alpha_0_h = degtorad(0);

Lf = convlength(fuse.total_fuselage_length,'in','m'); %fueselage length
Wf = convlength(fuse.d_f,'in','m'); % fueslage max width ( diameter)

% +ve if thrustline under cg
Zt = -0.716;

required_static_margin = .05*Cmac;

CD = .01;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
syms wing_xac_bar_syms kfx

Kf_gen; % interpolate kf_gen data

wing_root_quater_chord_percent_length = @(xac) (((0.25*wing_Croot) - wing.Xac_from_tip) + xac)./Lf;
Kf_fit = poly2sym(Kf_fit_coef,kfx);
Kf(wing_xac_bar_syms) =  subs(Kf_fit,kfx,wing_root_quater_chord_percent_length(wing_xac_bar_syms.*Cmac));

fuselage_Cm_alpha(wing_xac_bar_syms) = Kf.*(Lf.*(Wf.^2))./(Cmac .* Sw);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


xcg = @(xacw,xach,wf_fuel,payload_factor) 4.6;


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

syms lh_syms zh_syms kh deda cla_eta_syms
%set to constant for now
zh_syms = zh;
% lh_syms = lh;

ka = (1./wing_AR) - (1./(1 + (wing_AR.^1.7)));
klamb = (10-(3.*wing.lambda))./7;
kh(lh_syms) = (1- abs(zh_syms./wing_b))./(((2.*lh_syms)./(wing_b)).^(1/3));

deda(lh_syms,cla_eta_syms) = 4.44 .* ((ka .* klamb .* kh .* sqrt(cosd(wing_sweep_25))).^1.19).*(cla_eta_syms);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%CM0W%%%%%%%
syms Cm0w 

Cm0_airfoil = -0.075;
Cm0w(cla_eta_syms) = ((Cm0_airfoil.* (wing_AR .* (cosd(wing_sweep_25)^2)) ./ (wing_AR+(2*cosd(wing_sweep_25)))) - (0.01 .* wing.twist)) .* (cla_eta_syms); 

%%%%%%%%%%%%%%%%

%%%%REGIONS%%%%%%
region.regions = ["Takeoff","Landing","Cruise Start","Cruise End"];
region.cla_eta = [aero_analysis.wing.eta([3 4 1 1])];
% region.deda = subs(deda,cla_eta_syms,region.cla_eta);

region.fuel_fraction = [sizing.fraction.before_take_off,sizing.fraction.before_alternate_cruise,sizing.fraction.before_cruise,sizing.fraction.end_cruise_1];

% region.xcg = []; % -> cams function
region.cl_alpha_w = [ aero_analysis.wing.HLD.cl_alpha_flaps([1 2]) aero_analysis.wing.Cl_alpha([1 1]) ];
region.Cm0w = double(subs(Cm0w,cla_eta_syms,region.cla_eta));

region.CL = [0.3 0.3 0.3 0.3]; % design lift coefficents at specific regions
region.rho = [1.225,1.225,0.3016,0.3016];
% region.a = [340.3,340.3,295.0696,295.0696];
% region.m = [.1,.1,.75,.75];
region.v = [aero_analysis.wing.v_takeoff_ms,aero_analysis.wing.v_landing_ms,221.3022,221.3022];
region.q = 0.5 * (region.rho .* (region.v.^2));

region.C_thrust = [design.t0/(region.q(1)*Sw*Cmac),(design.t0/(region.q(1)*Sw*Cmac)) * 0.5,CD/Cmac,CD/Cmac]; % T/(q*sw*cbar) -> takeoff = 100% thrust, landing = 50% thrust
% region.alpha_0_w = degtorad([-3 -3 -3 -3]);
% region.alpha_0_h = degtorad([0 0 0 0]);



%%%%%%%%%%%%%%%%%

% syms xcg_xac_bar 
%tailplane stability analysis
% static_stab_constraint_sh_sw(xcg_xac_bar) = (wing_Cl_alpha * xcg_xac_bar)/(tail_h_Cl_alpha*eta_h*(1-deda)*((lh/Cmac)-xcg_xac_bar));
% control_stab_constraint_sh_sw(xcg_xac_bar) = 


syms xnp_bar wing_xac_bar_syms sh_sw_syms tail_h_xac_bar_syms lh_eq sm wing_Cl_alpha_syms wf_fuel_syms payload_factor_syms

xcg_bar_eq(wing_xac_bar_syms,tail_h_xac_bar_syms,wf_fuel_syms,payload_factor_syms) = (sym(xcg(wing_xac_bar_syms,tail_h_xac_bar_syms,wf_fuel_syms,payload_factor_syms)./Cmac));

lh_eq(tail_h_xac_bar_syms,wing_xac_bar_syms) = tail_h_xac_bar_syms - wing_xac_bar_syms;

xnp_bar(sh_sw_syms,wing_xac_bar_syms,tail_h_xac_bar_syms,wing_Cl_alpha_syms,cla_eta_syms) = ((wing_Cl_alpha_syms .* wing_xac_bar_syms) - fuselage_Cm_alpha(wing_xac_bar_syms) + (eta_h .* sh_sw_syms .* tail_h_Cl_alpha .* (1-deda(lh_eq,cla_eta_syms)).*tail_h_xac_bar_syms)) ./ (wing_Cl_alpha_syms + (eta_h.*tail_h_Cl_alpha.*(1-deda(lh_eq,cla_eta_syms)).*sh_sw_syms));
%payload conf
sm(sh_sw_syms,wing_xac_bar_syms,tail_h_xac_bar_syms,wing_Cl_alpha_syms,cla_eta_syms,wf_fuel_syms,payload_factor_syms) = xnp_bar(sh_sw_syms,wing_xac_bar_syms,tail_h_xac_bar_syms,wing_Cl_alpha_syms,cla_eta_syms) - xcg_bar_eq(wing_xac_bar_syms,tail_h_xac_bar_syms,wf_fuel_syms,payload_factor_syms);

disp('static margin')

double(sm(Sh/Sw,locations.x_ac_w/Cmac,locations.x_ac_h/Cmac,region.cl_alpha_w(1),region.cla_eta(1),sizing.fraction.before_take_off,1))
double(sm(Sh/Sw,locations.x_ac_w/Cmac,locations.x_ac_h/Cmac,region.cl_alpha_w(2),region.cla_eta(2),sizing.fraction.end_cruise_1,1))
double(sm(Sh/Sw,locations.x_ac_w/Cmac,locations.x_ac_h/Cmac,region.cl_alpha_w(3),region.cla_eta(3),sizing.fraction.before_cruise,1))
double(sm(Sh/Sw,locations.x_ac_w/Cmac,locations.x_ac_h/Cmac,region.cl_alpha_w(4),region.cla_eta(4),sizing.fraction.end_cruise_1,1))

double(xnp_bar(Sh/Sw,locations.x_ac_w/Cmac,locations.x_ac_h/Cmac,region.cl_alpha_w(1),region.cla_eta(1)))
double(xnp_bar(Sh/Sw,locations.x_ac_w/Cmac,locations.x_ac_h/Cmac,region.cl_alpha_w(2),region.cla_eta(2)))
double(xnp_bar(Sh/Sw,locations.x_ac_w/Cmac,locations.x_ac_h/Cmac,region.cl_alpha_w(3),region.cla_eta(3)))
double(xnp_bar(Sh/Sw,locations.x_ac_w/Cmac,locations.x_ac_h/Cmac,region.cl_alpha_w(4),region.cla_eta(4)))

figure
hold on

xmin = 0;
xmax = 5;
ymin = 0;
ymax = 1;

% takeoff max fuel and passengers
f1 = matlabFunction(subs(sm,[tail_h_xac_bar_syms,wing_Cl_alpha_syms,cla_eta_syms,wf_fuel_syms,payload_factor_syms],[tail_h_xac_bar,region.cl_alpha_w(1),region.cla_eta(1),sizing.fraction.before_take_off,1]));
%lb sm > 0%
fimplicit(@(x,y) f1(y,x),[xmin xmax ymin ymax],'o-','color','red') 
% ub sm < 20%
fimplicit(@(x,y) f1(y,x) - .2,[xmin xmax ymin ymax],'x-','color','red') 
% ideal lb sm > 4%
fimplicit(@(x,y) f1(y,x) - .04,[xmin xmax ymin ymax],'+-','color','red') 
%ideal ub sm < 7%
fimplicit(@(x,y) f1(y,x) - .07,[xmin xmax ymin ymax],'.-','color','red') 

%landing empty fuel and passengers?
f2 = matlabFunction(subs(sm,[tail_h_xac_bar_syms,wing_Cl_alpha_syms,cla_eta_syms,wf_fuel_syms,payload_factor_syms],[tail_h_xac_bar,region.cl_alpha_w(2),region.cla_eta(2),sizing.fraction.end,1]));
%lb sm > 0%
fimplicit(@(x,y) f2(y,x),[xmin xmax ymin ymax],'o-','color','blue') 
% ub sm < 20%
fimplicit(@(x,y) f2(y,x) - .2,[xmin xmax ymin ymax],'x-','color','blue') 
% ideal lb sm > 4%
fimplicit(@(x,y) f2(y,x) - .04,[xmin xmax ymin ymax],'+-','color','blue') 
%ideal ub sm < 7%
fimplicit(@(x,y) f2(y,x) - .07,[xmin xmax ymin ymax],'.-','color','blue') 


%cruise moderate fuel and full passengers
f3 = matlabFunction(subs(sm,[tail_h_xac_bar_syms,wing_Cl_alpha_syms,cla_eta_syms,wf_fuel_syms,payload_factor_syms],[tail_h_xac_bar,region.cl_alpha_w(3),region.cla_eta(3),sizing.fraction.end_cruise_1,1]));
%lb sm > 0%
fimplicit(@(x,y) f3(y,x),[xmin xmax ymin ymax],'o-','color','green') 
% ub sm < 20%
fimplicit(@(x,y) f3(y,x) - .2,[xmin xmax ymin ymax],'x-','color','green') 
% ideal lb sm > 4%
fimplicit(@(x,y) f3(y,x) - .04,[xmin xmax ymin ymax],'+-','color','green') 
%ideal ub sm < 7%
fimplicit(@(x,y) f3(y,x) - .07,[xmin xmax ymin ymax],'.-','color','green') 

yline(Sh/Sw)
xline(xacw/Cmac)

legend('Sm > 0%','Sm < 20%','Sm > 4%','Sm < 7%')
ylabel('{S_h}/{S_w}')
xlabel('{x_ac_w}/{Cmac}')
grid on

improvePlot(gcf)


%% TRIMMED CG ANALYSIS

syms CL_w CL_h alpha iw_syms ih_syms C_thrust_syms



Cmalphaf = fuselage_Cm_alpha(wing_xac_bar);
sh_sw = Sh/Sw;

CL_w(alpha,iw_syms,wing_Cl_alpha_syms) = wing_Cl_alpha_syms.*(alpha + iw_syms - alpha_0_w);
CL_h(alpha,ih_syms,iw_syms,cla_eta_syms) = tail_h_Cl_alpha.*( ((alpha + iw_syms - alpha_0_w).*(1-deda(lh,cla_eta_syms))) + (ih_syms-iw_syms) - (alpha_0_h - alpha_0_w));


Cmcg(alpha,ih_syms,iw_syms) = -CL_w(alpha,iw_syms,wing_Cl_alpha_syms).*(wing_xac_bar_syms - xcg_bar_eq(wing_xac_bar_syms,tail_h_xac_bar_syms,wf_fuel_syms,payload_factor_syms)) + Cm0w(cla_eta_syms) + (Cmalphaf.*alpha) - (eta_h.*CL_h(alpha,ih_syms,iw_syms,cla_eta_syms).*sh_sw.*(tail_h_xac_bar - xcg_bar_eq(wing_xac_bar_syms,tail_h_xac_bar_syms,wf_fuel_syms,payload_factor_syms))) + (Zt.*C_thrust_syms);
CL(alpha,ih_syms,iw_syms) = CL_w(alpha,iw_syms,wing_Cl_alpha_syms) + (eta_h.*sh_sw.*CL_h(alpha,ih_syms,iw_syms,cla_eta_syms));


for idx = 1:length(region.regions)

    %sub in aerodymaic constants
    Cmcg_function = subs(Cmcg,[cla_eta_syms,wing_Cl_alpha_syms,wing_xac_bar_syms,tail_h_xac_bar_syms],[region.cla_eta(idx),region.cl_alpha_w(idx),wing_xac_bar,tail_h_xac_bar])
    % sub in for weight configuration -> assuming always fully loaded with payload
    Cmcg_function = subs(Cmcg_function,[wf_fuel_syms,payload_factor_syms],[region.fuel_fraction(idx),1]);
    % sub in thrust settings
    Cmcg_function = subs(Cmcg_function,[C_thrust_syms],[region.C_thrust(idx)]);
    % expose the variables we want
    Cmcg_function(alpha,ih_syms,iw_syms) = matlabFunction(Cmcg_function,'vars',[alpha,ih_syms,iw_syms]);

    CL_function(alpha,ih_syms,iw_syms) = matlabFunction(subs(CL,[cla_eta_syms,wing_Cl_alpha_syms],[region.cla_eta(idx),region.cl_alpha_w(idx)],'vars',[alpha,ih_syms,iw_syms]);

    %x(1) = alpha, x(2) = ih
    F(1) = @(x) Cmcg_function(x(1),x(2),wing.i_w);
    F(2) = @(x) CL_function(x(1),x(2),wing.i_w) - region.CL{idx};
    x0 = [0,0];
    res = fsolve(F,x0)

end